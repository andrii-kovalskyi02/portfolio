    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-start mb-4">
          ...
          <button onClick={onClose} aria-label="Close" className="text-gray-400 hover:text-black text-2xl leading-none">×</button>
        </div>

        <div className="flex flex-col items-center gap-4 py-6">
          <div aria-hidden className={`w-24 h-24 rounded-full flex items-center justify-center text-4xl transition-all ${isActive ? 'bg-red-100 animate-pulse' : 'bg-gray-100'}`}>
            {isActive ? '🔴' : '🎙️'}
          </div>
...
export default function AskMyCVDialog({ onClose }: { onClose: () => void }) {
  return (
    <ConversationProvider>
      <AskMyCVInner onClose={onClose} />
    </ConversationProvider>
  )
}
